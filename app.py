...
def start_play():
    while True:
        ...
        if str.isdigit(game_choice) and 0 < int(game_choice) < 4:
            ...
            while True:
                difficulty = input('Now, choose your a difficulty between 1-5 :')

                if difficulty.isdigit() and 1 <= int(difficulty) <= 5:
                    difficulty = int(difficulty)
                    print(f'You selected difficulty {difficulty}. Lets begin!')
                    break
                else:
                    print('choose your a number between 1-5 only')
